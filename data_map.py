...
def global_total_map():
    ...
    pieces = [
        ...
        {'max':9999999,'min':1000000,'label':'>=1000000','color':'#BA0808'}
        ]
    name_map = {
        "Somalia": "索马里",
        "Liechtenstein": "列支敦士登",
...
def all_map():
    page = Page(layout = Page.SimplePageLayout,page_title = '疫情实时大数据')
    page.add(
        china_total_map()[0],china_total_map()[1],
        global_total_map()[0],global_total_map()[1],
        china_daily_map(),
        foreign_daily_map()
        )
    page.render('COVID-19.html')
    print('国内及境外可视化图表绘制完毕')
